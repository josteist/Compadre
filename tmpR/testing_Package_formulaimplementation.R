...
ma <- make_BayesCMR_new(Obs,dts,
                        spec= ~d180 + d13C+time,
                        ext = ~d180 + d13C+time+div,
                        samp = ~time,
                        data = drivers)
mb <- make_BayesCMR(Obs,dts,DivDep=c(F,T),
                    RE=c(T,T,T),ExtTS = t(drivers[1:12,1:2]),
                    SpecTS = t(drivers[1:12,1:2]))
xa = runif(ma$npar,min=-0.03,max=0.03)
xa[unlist(ma$inx[5:7])] = 0
xa[1:3] = runif(3,min=-3,max=-2)
# drivers and vars are wrong, so ALL
# xb[4:max(unlist(mb$aix))] #should change
#
# THere's something with the div'dep that isn't quite right
# xa[7] = 0
xb = xa;
unlist(mb$aix)
ma$inx[1:3]
xb[c(8:9)] = xa[c(4,5)]
xb[c(10:11,7)] = xa[c(6,7,8)]
xb[4:6] = xa[ma$inx$varInx]
# xb[10:11]  = xa[7:8]
...
ma <- make_BayesCMR_new(Obs,dts,spec=~time+SeaLev*div,
                        samp= ~time,data=drivers)
fa <- MCMC_CMR(ma,niter=5e5)
fb <- MCMC_CMR(mb,niter=1e5)

par(mfrow=c(2,3))
sapply(fa$Model$inx$specInx,
       function(ii){hist(fa$Chain[-c(1:dim(fa$Chain)[2]/2),ii])})
hist(fa$Chain[-c(1:dim(fa$Chain)[2]/2),fa$Model$inx$specInx])

xa = c(-1,-1.5,-2,rep(0,ma$npar-3))
xb = xa;
...
sum(unlist(ma$priorf(xa)[4:15]))
mb$priorfun[[1]](xb)
# SO the RE's are seemingly ok.
# Clooooser.
...
ma <- make_BayesCMR_new(Obs,dts,
                        ...
                        samp = ~time,
                        data = drivers)
ma$priorf(runif(ma$npar))
fa <- MCMC_CMR(ma,niter=1e5)
# FUnction to quickl get ESSs
sapply(unlist(ma$inx[1:4]),
       function(ii){coda::effectiveSize(fa$Chain[-c(1:dim(fa$Chain)[1]/2),ii])})
matplot(fa$Chain[,1:3],type="l")


system.time(replicate(2000,ma$probfun(runif(ma$npar))))/
system.time(replicate(2000,mb$probfun(runif(mb$npar))))
# So new is slightly more time-consuming, perhaps 20%.
...
matplot(t(sapply(seq(-.5,.5,by=0.01),function(ii){
  ma$specfun(c(x[1:(jj-1)],ii,x[(jj+1):length(x)]))})),type="l")
par(mfrow=c(2,3))
for (ii in seq(-1,1,length.out=6)){
plot(ma$dataSE$d13C,ma$specfun((c(x[1:5],ii,x[7:length(x)]))))
}
...
plot(ma$extfun(xa),mb$extfun(xb))
