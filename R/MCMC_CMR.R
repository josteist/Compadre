...
#'
#' @param cmrModel is a \code{CMR_model} generated using \link{make_BayesCMR}
#' @param niter is number of iterations. Defaults to \code{niter = 1e3}. Default is generally only sufficient for models with only mean rates and no drivers.
#' @param nthin sets the thinning, i.e. samples for each \code{nthin} iterations are stored as output. Defaults to \code{nthin = 10}.
#' @param vmin sets the minimum and initial standard deviation of the normal proposals.
#' @param draweps number of iterations between each update of the proposal covariance matrix. Defaults to \code{draweps = niter/100}.
#' @param x_init Potential initial values for the chain. If not provided the initial mean rates are optimized using the probability density function and all other parameters set to 0.
#' @param cvstp Covariance structure for proposal. This is tuned during burnin phase. Defaults to (2.38/(sqrt(cmrModel$npar)))^2*diag(vmin,cmrModel$npar))
#' @param adapt Adapt the covariance proposals during first half?
#' @return A \code{CMR_fit} structure with \code{$Chain} for samples, \code{$Probs} for posterior probabilities, \code{$Accept} with number of accepted proposals in each block, \code{$Model} is the \code{CMR_model} supplied as input, \code{$Covs} is the proposal covariance structure used in the last half of the chain.
#' @export
#'
