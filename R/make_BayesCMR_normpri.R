...
#' @param priorsNorm_Var sets the parameters for the normal prior for the log variances of the random efSpecTS. Same for all three variances, Norm(0,3/1.96)
...
# of sampling rates.
make_BayesCMR_normpri <- function(Obs,dts=rep(1,dim(Obs)[2]),
                                  spec =  ~ 1,
                                  ext  =  ~ 1,
                                  samp =  ~ 1,
                                  data = NULL,
                                  ...
                                  priorsNorm_Var=c(0,3/1.96),replRE_1 = F){
  ...
  # priorsUnif - now this is norm(par[1],par[2])
  # priorsNorm_Cov - prior parameters for all non-variance parameters (mean[rates], all covariate efSpecTS and divdep)
  # priorsNorm_Var - prior parameters for variance terms [4:6] for random efSpecTS dunif([1],[2])
  ...
  myll <- function(x){
    ...
    # _se
    ll <- pradel_unvd_gam(
      ext = (exp(lext)*((exp((exp(lfec)-exp(lext))*dts_se))-1))/
          (exp(lfec)*((exp((exp(lfec)-exp(lext))*dts_se)))-exp(lext)),
      gam <- (1-(exp(lext)*((exp((exp(lfec)-exp(lext))*dts_se))-1))/
           (exp(lfec)*((exp((exp(lfec)-exp(lext))*dts_se)))-exp(lext)))/
          exp((exp(lfec)-exp(lext))*dts_se),
      p   = rate2prob(exp(lsmp),dts),
      u,n,v,d);
  ...
  if (any(RE)){
    prfun <- function(x){
      c(sapply(1:3,function(ii){dnorm(x[ii],priorsNorm_Mus[[ii]][1],priorsNorm_Mus[[ii]][2],log=T)}),
        dnorm(x[alphinx$varInx],priorsNorm_Var[1],priorsNorm_Var[2],log=T),
