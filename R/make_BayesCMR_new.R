...
# of sampling rates.
make_BayesCMR_new <- function(Obs,dts=rep(1,dim(Obs)[2]),
                              spec =  ~ 1,
                              ext  =  ~ 1,
                              samp =  ~ 1,
                              data = NULL,
  ...
  if (is.null(Zspec)){# If no RE for speciation
    if (dim(mmspec)[2]==1){# if only intercept
      ...
      if (updmmx[1]){
        ...
        lspecfun <- function(x){
          mmspec[,which(colnames(mmspec)=='div')] = n_norm(x);
          return(rowSums(cbind(mmspec %*% x[alphinx$specInx])))        }
      } else {
  ...
  if (is.null(Zext)){# If no RE for speciation
    if (dim(mmext)[2]==1){# if only intercept
      ...
      if (updmmx[2]){
        # If div-dep, fill in with divers
        lextfun <- function(x){
          mmext[,which(colnames(mmext)=='div')] = n_norm(x);
          return(rowSums(cbind(mmext %*% x[alphinx$extInx])))        }
      } else {
        lextfun <- function(x){rowSums(cbind(mmext %*% x[alphinx$extInx]))}
      }
    }
  } else {# if RE's
  ...
  prfun <- function(x){
    c(sapply(1:3,function(ii){dnorm(x[ii],priorsNorm_Mus[[ii]][1],priorsNorm_Mus[[ii]][2],log=T)}),
      ...
      sapply(sum(RE),function(ii){dnorm(   x[alphinx[[4+which(RE)[ii]]]],0,alphinx$varInx[ii],log=T)}))}

  probfun <- function(x){myll(x)$LogL+sum(prfun(x))}
  ...
  out <- list(probfun= probfun,likfun=myll,priorf  = prfun,
              ...
              mmsamp=mmsamp,mmspec=mmspec,mmext=mmext,
