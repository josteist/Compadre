...
plotRates <- function(cmrfit,max_ma= NULL,stages =NULL,logax = T,draws = 250,quantiles = c(0.025,0.5,0.975),plot=T,cladenames=NULL){
  ...
  if (is.null(cmrfit$Model$Clade1Mod)){
    ...
    invisible(plotfun_internal(spec_smps1,ext_smps1,smp_smps1,bnds,tmpl,stages,quantiles=quantiles,main=cladenames[[1]]))
    invisible(plotfun_internal(spec_smps2,ext_smps2,smp_smps2,bnds,tmpl,stages,quantiles=quantiles,main=cladenames[[2]]))
